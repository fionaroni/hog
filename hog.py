...
def roll_dice(num_rolls, dice=six_sided): # completely tested!
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0: # free bacon
        largest_digit_in_opponent_score=max([int(c) for c in str(opponent_score)])
        turn_score = 1 + (largest_digit_in_opponent_score)
        return turn_score
    else:
...
def select_dice(score, opponent_score):
    ...
    True
    """
    if add(score, opponent_score) % 7 == 0: # hog wild
        return four_sided
    else:
        return six_sided
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < goal and opponent_score < goal:
        dice = select_dice(score, opponent_score) # hog wild
        if who == 0:
            ...
            who = other(who)
        if (score == 2 * opponent_score) or (opponent_score == 2 * score): # swine swap
...
def make_averaged(fn, num_samples=5000): #change num_samples back to 1000
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    10
    """
    num_rolls_with_max_av = 0
    maximum_average = 0
    num_rolls = 1
    while num_rolls <= 10:
        average = make_averaged(roll_dice, 1000)(num_rolls, dice)
        print(num_rolls, "dice scores", average, "on average")
        if maximum_average < average:
            maximum_average = average
            num_rolls_with_max_av = num_rolls
        num_rolls += 1
    return num_rolls - 1

    # is this return value correct? (I'm guessing it's correct so long as number of rolls w/ max average is 10. Isn't this always the case regardless of doctest?)
...
def bacon_strategy(score, opponent_score):
    ...
    """
    largest_digit_in_opponent_score=max([int(c) for c in str(opponent_score)])
    if largest_digit_in_opponent_score + 1 >= BACON_MARGIN:
        return 0
    else:
...
def swap_strategy(score, opponent_score):
    ...
    """
    largest_digit_in_opponent_score=max([int(c) for c in str(opponent_score)])
    new_score = score + (largest_digit_in_opponent_score + 1)
    if new_score * 2 == opponent_score: # favorable swap
        return 0
    elif opponent_score * 2 == new_score: # harmful swap
        return BASELINE_NUM_ROLLS
    elif largest_digit_in_opponent_score + 1 >= BACON_MARGIN:
        return 0
    else:
        return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    """ 
    1. First, check for "ideal" situations. If rolling 0 leads to a favorable
    swap AND makes opponent get 4-sided dice on next turn, roll 0. If rolling 0 
    leads to a favorable swap (but does not make opponent get 4-sided dice on next
    turn), still roll 0.
    2. If I am in the lead, take fewer risks and emphasize defense. Check if 
    rolling 0 will result in a harmful swap. If so, return BASELINE_NUM_ROLLS. If 
    not, check if adding one to the largest digit in the opponent's score
    will be greater than or equal to the BACON_MARGIN. If so, roll 0. If not, roll
    BASELINE_NUM_ROLLS.
    3. If I am losing, check how badly I'm losing. If the difference between score
    and opponent_score is greater than 24, roll 8. If not, roll BASELINE_NUM_ROLLS. 
    """
    largest_digit_in_opponent_score=max([int(c) for c in str(opponent_score)])
    new_score = score + (largest_digit_in_opponent_score + 1)
    if new_score * 2 == opponent_score or largest_digit_in_opponent_score + 1 >= BACON_MARGIN: # favorable swap AND forces opponent to roll 4 sided dice
        return 0
    elif new_score * 2 == opponent_score: # favorable swap
        return 0
    else:
        if score > opponent_score: # if winning
            if score > 98:
              return 10
            if score > 80: # if very close to winning
                return 5
            elif opponent_score * 2 == new_score: # if returning 0 results in a harmful swap, don't return 0
                return 4
            elif largest_digit_in_opponent_score + 1 >= BACON_MARGIN:
                return 0
            else:
            ...
            if opponent_score - score > 24: # if losing significantly, take bigger risk to catch up
                return 6
            else:
                return 6
